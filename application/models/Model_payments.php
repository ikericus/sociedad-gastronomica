...
class Model_payments extends CI_Model
{
	...
	public function getPaymentsItemData($payment_id = null)
	{
		if(!$payment_id) {
			return false;
		}
	...
	public function create($user_id, $amount, $date_time = null)
	{
    	$data = array(
    		'date_time' => $date_time ?? strtotime(date('Y-m-d h:i:s a')),
    		'amount' 	=> $amount,
    		'user_id' 	=> $user_id
    	);
	...
	public function sumPaymentsUser($user_id)
	{
		if($user_id) {
