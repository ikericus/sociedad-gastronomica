...
class Orders extends Admin_Controller 
{
	...
	*/
	public function fetchOrdersData()
	{
		...
		foreach ($data as $key => $value) {
			...
			if($is_admin || in_array('viewOrder', $this->permission)) {
				$buttons .= '<a target="__blank" href="'.base_url('orders/printDiv/'.$value['id']).'" class="btn btn-default"><i class="fa fa-print"></i></a>';
			}

			if($is_admin || (in_array('updateOrder', $this->permission) and $user_id == $value['user_id'])) {
				$buttons .= ' <a href="'.base_url('orders/update/'.$value['id']).'" class="btn btn-default"><i class="fa fa-pencil"></i></a>';
			}

			if($is_admin || (in_array('deleteOrder', $this->permission) and $user_id == $value['user_id'])) {
	...
	*/
	public function create()
	{
		...
		$this->data['page_title'] = 'Crear sobre';

		$this->form_validation->set_rules('product[]', 'Nombre del producto', 'trim|required');
		$this->form_validation->set_rules('net_amount_value', 'Total', 'trim|required');
        ...
        if ($this->form_validation->run() == TRUE) {        	
        	...
        	if($order_id) {
        		$this->session->set_flashdata('success', 'Sobre creado con éxito');
				//redirect('orders/update/'.$order_id, 'refresh');
				redirect('orders/index');
        	}
        	else {
        		$this->session->set_flashdata('errors', 'Ha ocurrido un error al crear el sobre');
	...
	*/
	public function update($id)
	{
        ...
        if ($this->form_validation->run() == TRUE) {        	
        	...
        	if($update == true) {
        		$this->session->set_flashdata('success', 'Sobre actualizado');
				redirect('orders/index');
        	}
        	else {
	...
	public function remove()
	{
        ...
        if($order_id) {
            ...
            if($delete == true) {
                ...
                $response['messages'] = "Sobre eliminado con éxito"; 
            }
            else {
                $response['success'] = false;
                $response['messages'] = "Ha ocurrido un error al eliminar el sobre";
            }
        }
        else {
            $response['success'] = false;
	...
	*/
	public function printDiv($id)
	{
		...
		if($id) {
			...
			$order_date = date('Y-m-d H:i:s', $order_data['date_time']);
			  ...
			  <section class="invoice">
			    ...
			    <div class="row">
			      <div class="col-xs-12">
			        <h2 class="page-header">
			          '.strtoupper($this->config->item('full_name')).'
			    ...
			    <div class="row invoice-info">
			      ...
			      <div class="col-sm-4 invoice-col">
			        <b>ID sobre: </b> '.$order_data['bill_no'].'<br>
